...
public class Argon2ID extends PluginHash implements LegacyPluginHash {
    ...
    public boolean needsRehash(final HashResult result) {
        ...
        return argon.needsRehash(product, encryption.iterations(), encryption.memory(), encryption.parallelism())
                || !encryption.algorithm().equalsIgnoreCase("argon2id") || result.product().valid() != encryption.virtualID();
    }
